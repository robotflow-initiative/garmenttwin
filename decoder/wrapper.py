...
def decode_thread(index, capture, p: PostProcessor, camera_name:str) -> Optional[Dict]:
    # Get color image
    ...
    if not ret_color or not ret_depth:
        ...
        p.detect_one(camera_name, color_image, depth_image, index)
...
def mkv_record_wrapper(path: str, p: PostProcessor) -> Tuple[Optional[concurrent.futures.Future], Optional[Exception]]:
    if not path.endswith('.mkv'):
    ...
    while True:
        try:
            ...
            if not ret:
                ...
                time.sleep(0.01)
                yield pool.submit(decode_thread, index, capture, p, camera_name), None
        except EOFError:
            pool.shutdown(wait=True)
            return None, EOFError('end of file')
