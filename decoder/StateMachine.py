...
def state_machine_save_thread_v1(m: StateMachine, p: PostProcessor, data_dir: str, camera_names: List[str]):
        ...
        if m.ready:
            ...
            if frames is not None:
                for stream_id, frame in frames.items():
                    color_path = osp.join(data_dir, stream_id, 'color', f'{frame["index"]}.jpg')  # "color_dev_ts_usec"
                    depth_path = osp.join(data_dir, stream_id, 'depth', f'{frame["index"]}.png')
                    # pool.submit(p.detect_one(stream_id, frame['color'], frame['depth'], frame["index"]))
